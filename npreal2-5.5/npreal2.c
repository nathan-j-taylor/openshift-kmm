...
#if (OS_TYPE==ROCKY_ID || OS_TYPE==REDHAT_ID || OS_TYPE==CENTOS_ID || OS_TYPE==ALMA_ID)
#undef _PDE_DATA
#if (LINUX_VERSION_CODE >= VERSION_CODE(5,14,0) && DIS_PATCH!=70)
#define _PDE_DATA pde_data
#else
#define _PDE_DATA PDE_DATA
#endif
...
#if ((OS_TYPE==REDHAT_ID || OS_TYPE==ROCKY_ID || OS_TYPE==CENTOS_ID || OS_TYPE==ALMA_ID) && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0) && DIS_PATCH >= 450) 
static ssize_t npreal_write(struct tty_struct *,const unsigned char *,size_t);
#else
...
npreal_init(void)
{
    ...
    ((OS_TYPE==REDHAT_ID || OS_TYPE==ROCKY_ID || OS_TYPE==CENTOS_ID || OS_TYPE==ALMA_ID)  && LINUX_VERSION_CODE == KERNEL_VERSION(5, 14, 0) && DIS_PATCH >= 450)
		...
		struct ktermios *old_termios)
{
	...
	case B0:
		...
		npreal_linectrl(nd,info->modem_control);
		return 0;
	default:
